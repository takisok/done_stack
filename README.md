...
できたスタンプ💮 は、今日できたことを記録して、できた実感を残すローカル用のメモアプリです。
メイさんとキーたんが、記録した達成を一緒に見守ってくれます。
...
できたスタンプ💮 は静的なブラウザアプリです。サーバー側にユーザーデータを保存せず、記録データは基本的にブラウザ内の IndexedDB に保存されます。
...
`index.html`、`about.html`、`history.html`、`search.html`、`settings.html` には Content Security Policy を設定しています。
...
第三者ライブラリの表示は `THIRD_PARTY_NOTICES.md` にまとめています。

## ライセンス

本アプリ本体のソースコードと素材は、特に明記がない限り All rights reserved です。
